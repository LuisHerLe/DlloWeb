...
var agenda = {
  init: function(){
    ...
    /*sessionStorage.setItem("sitios", JSON.stringify(this.sitiosGuardados)) //OJO::: Almacenar con sessionStorage*/
    this.loadSites() //Almacenar con localStorage
  },
  ...
  saveAndPlaceMarker: function(site){
    if(localStorage.sitios){
      this.sitiosGuardados = JSON.parse(localStorage.getItem("sitios")) //OJO:: Para localStorage
    }
    /*this.sitiosGuardados = JSON.parse(sessionStorage.getItem("sitios"))//OJO:: Para sessionStorage*/
    this.sitiosGuardados.push(site)
    /*sessionStorage.setItem("sitios",JSON.stringify(this.sitiosGuardados))//OJO:: Para sessionStorage*/
    localStorage.setItem("sitios",JSON.stringify(this.sitiosGuardados))//OJO:: Para sessionStorage
    this.renderSite(site)
  },
  renderSite: function(site){

    var worker = new Worker('doHtml.js') //Se crea el nuevo worker con el nombre del archivo donde está almacenado
    worker.postMessage(site) //Se envía los datos del sitio al worker y este construirá el htmlInfo
